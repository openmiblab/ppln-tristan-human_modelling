...
def _line_plots_combined(src, ax1, ax2, ylim=[50,5]):

    file = os.path.join(src, 'Results', 'all_results')
...
def _effect_box_plots_combined(results, ax):

    drug = os.path.basename(results)
    file = os.path.join(results, 'Results', 'all_results')
    pars = pydmr.read(file)['pars']
    subjects = sorted(list({k[0] for k in pars}))
    all_data = [
    ...
    ax.yaxis.grid(True)
    ax.set_ylabel('Relative effect size (%)', fontsize=fontsize)



def effect_plot_combined(results, ylim=[30,3]):

    # Check if the result exists
    figpath = os.path.join(results, 'Figures')
    ...
    # If not compute it and save
    fig, (ax0, ax1, ax2) = plt.subplots(
        1, 3, width_ratios=[2, 4, 4], figsize=(8,3)
    )
    fig.subplots_adjust(wspace=0.5)
    _effect_box_plots_combined(results, ax0)
    _line_plots_combined(results, ax1, ax2, ylim=ylim)
...
def diurnal_k_combined(datafile, resultspath, ylim=[30,30,3,3]):

    # Check if result already exists
    path = os.path.join(resultspath, 'Figures')
    ...
    # Create line plots
    output_file = os.path.join(resultspath, 'Results', 'all_results')
