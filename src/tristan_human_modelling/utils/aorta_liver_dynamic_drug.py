...
class AortaLiverDynamicDrug(SuperModel):
    ...
    def _params(self, select=None):
        ...
        pars_list = {
            ...
            'free_aorta_control': inflow_pars + [
                'CO', 'Thl', 'Dhl', 'To', 'To_e', 'Eo',
                'c_BAT_1',  
                'c_BAT_2',
                'c_S0_2_a',
            ...
            'free_liver_control_1': [
                'Tg', 'Dg', 've', 
                'c_khe_i', 'c_kbh',
            ],
            'free_liver_control_2': [
                'c_khe_f',
                'c_kbh', 
                'c_S0_2_l',
            ],
    ...
    def _conc_aorta(self, visit, scans):
        ...
        khe_i, khe_f = p[f'{visit}_khe_i'], p[f'{visit}_khe_f']
        khe = khe_i + (khe_f - khe_i) * t / t.max()
    ...
    def _predict_liver_control_1(self, time):
        Sl = self._predict_liver_control(time, scans=1)
        return Sl
    
    def _predict_liver_control_2(self, time):
        Sl = self._predict_liver_control(time, scans=2)
        return Sl
    
    def _predict_liver_drug_1(self, time):
        Sl = self._predict_liver_drug(time, scans=1)
        return Sl
    
    def _predict_liver_drug_2(self, time):
        Sl = self._predict_liver_drug(time, scans=2)
        return Sl
    ...
    def _estimate_parameters(
        ...
        for i, visit in enumerate(['c', 'd']):
            ...
            def estimate_s0(scan, roi, i0, R10):
                ...
                s_ref = Signal(roi_seq[roi], **p)(R1=R10, R2s=R20s, S0=1, B1corr=B1, FA=FA)
    ...
    def _recalibrate(self, time, signal, n0):
        p = self._pars
        seq = self._cnfg['sequence']
        roi_seq = {
            'a': seq,
        ...
        for i, visit in enumerate(['c', 'd']):
            ...
            def estimate_s0(scan, roi, i0, R10): # same as in estimate - remove dupl
                B1=p[f'{visit}_B1corr_{scan}_{roi}']
                FA=p[f'{visit}_FA_{scan}']
                R20s=p[f'{visit}_R20s_{roi}']
                s_ref = Signal(roi_seq[roi], **p)(R1=R10, R2s=R20s, S0=1, B1corr=B1, FA=FA)
            ...
            if visit=='c':
                ts = self._time(visit) >= time[1][0]
                R102a = np.mean(self._R1a_control[ts][:n0[i]])
                R102l = np.mean(self._R1l_control[ts][:n0[i]])
            if visit=='d':
                ts = self._time(visit) >= time[5][0]
                R102a = np.mean(self._R1a_drug[ts][:n0[i]])
                R102l = np.mean(self._R1l_drug[ts][:n0[i]])

            estimate_s0(2, 'a', 1, R102a)
            estimate_s0(2, 'l', 3, R102l)


    def _train(
            self, time: tuple, signal: tuple, free: dict, 
            bounds:dict, R102a: list, R102l: list, n0: list, 
            staged: int, sigma: tuple=None, **kwargs,
        ):
        p = self._pars
        ...
        if staged==0: # make this staged=1 or add nit keyword
            train(self._predict, time, signal, p, free, sigma=sigma, **kwargs)
            # self._recalibrate(time, signal, n0)
            # train(self._predict, time, signal, p, free, sigma=sigma, **kwargs)
        ...
        if staged==2: # This does not seem to work - remove

            # Train aorta control data
            sig = None if sigma is None else (sigma[0], sigma[1])
            t, s = (time[0], time[1]), (signal[0], signal[1])
            free_aorta = {k: v for k, v in free.items() if k in self._params('free_aorta_control')}
            aorta = train(self._predict_aorta_control, t, s, p, free_aorta, sigma=sig, **kwargs)

            # Train liver control
            sig = None if sigma is None else (sigma[2], sigma[3])
            t, s = (time[2], time[3]), (signal[2], signal[3])
            free_liver = {k: v for k, v in free.items() if k in self._params('free_liver_control')}
            liver = train(self._predict_liver_control, t, s, p, free_liver, sigma=sig, **kwargs)

            # Train aorta drug data
            sig = None if sigma is None else (sigma[4], sigma[5])
            t, s = (time[4], time[5]), (signal[4], signal[5])
            free_aorta = {k: v for k, v in free.items() if k in self._params('free_aorta_drug')}
            aorta = train(self._predict_aorta_drug, t, s, p, free_aorta, sigma=sig, **kwargs)

            # Train liver drug
            sig = None if sigma is None else (sigma[6], sigma[7])
            t, s = (time[6], time[7]), (signal[6], signal[7])
            free_liver = {k: v for k, v in free.items() if k in self._params('free_liver_drug')}
            liver = train(self._predict_liver_drug, t, s, p, free_liver, sigma=sig, **kwargs)
    ...
    def export_params(self) -> dict:
        ...
        self._set_time()
        dpars = _deriv_params(self._pars, self._t_control, self._t_drug)
...
def _deriv_params(pars, c_t, d_t):

    # TODO: return sdev of derived

    # c_Eb
    khe_i, khe_f = pars[f'c_khe_i'], pars[f'c_khe_f']
    khe = khe_i + (khe_f - khe_i) * c_t / c_t.max()
    CL = khe * pars[f'c_vol'] + pars['GFR']
    Eb = CL / (CL + pars['CO'] * (1 - pars['H']))
    C_Eb = np.mean(Eb)

    # d_Eb
    khe_i, khe_f = pars[f'd_khe_i'], pars[f'd_khe_f']
    khe = khe_i + (khe_f - khe_i) * d_t / d_t.max()
    CL = khe * pars[f'd_vol'] + pars['GFR']
    Eb = CL / (CL + pars['CO'] * (1 - pars['H']))
    D_Eb = np.mean(Eb)
