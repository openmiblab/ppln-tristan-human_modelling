...
#SBATCH --comment=rifampicin
#SBATCH --job-name=rifampicin
...
# Run scripts
srun $PYTHON "$BASE_DIR/wip/tristan_rifampicin_effect.py" --data="$DATA" --build="$BUILD" --scans=2
srun $PYTHON "$BASE_DIR/wip/tristan_rifampicin_effect.py" --data="$DATA" --build="$BUILD" --scans=1