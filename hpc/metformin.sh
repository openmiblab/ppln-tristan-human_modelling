...
#SBATCH --comment=metformin
#SBATCH --job-name=metformin
...
# Run scripts
srun $PYTHON "$BASE_DIR/wip/tristan_metformin_effect.py" --data="$DATA" --build="$BUILD" --scans=2
srun $PYTHON "$BASE_DIR/wip/tristan_metformin_effect.py" --data="$DATA" --build="$BUILD" --scans=1